...
function get_my_scores_by_drivers_id($id) {
  global $wpdb;
  $output = array();
  $races = get_my_races();
...
function get_my_races() {
  global $wpdb;
  $query = "SELECT * FROM app_carrera";
  $rs = $wpdb->get_results($query);
  //print_r($rs);
  //die();
  return $rs;
...
function thumb($url){
  $file = check_my_filename($url);
  $baseurl = check_my_baseurl($url);
  $file = replace_my_file($file, '-100x100.');
  return $baseurl . $file;
}

function replace_my_file($file, $mode){
  return str_replace( '.', $mode, $file);
}

function check_my_filename($url){
 $all = explode("/", $url);
 $file = $all[count($all)-1];
 return $file;
}

function check_my_baseurl($url){
 $all = explode("/", $url);
...
/* ONLINE */
/*function thumb($url){
 $url = check_file_name($url);
 $sectors = explode(".", $url);
 $ext = $sectors[count($sectors)-1];
 unset($sectors[count($sectors)-1]);
 $url_base = implode(".",$sectors);
 $full_url = $url_base . "-100x100." . $ext;
 return  crop_last($full_url);
}*/
...
/*function medium($url){
 ...
 unset($sectors[count($sectors)-1]);
 $url_base = implode(".",$sectors);
 $full_url = $url_base . "-300x300." . $ext;
 return  crop_last($full_url);
}*/

/*function check_file_name($url){
 $sectors = explode(".", $url);
 $ext = $sectors[count($sectors)-1];
 $all = explode("/", $url);
 $file = $all[count($all)-1];
...
function toolbox_posted_on() {
	printf( __( '<span class="sep">Publicado el </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="byline"> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'toolbox' ),
